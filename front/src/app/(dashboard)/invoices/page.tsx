...
export default function InvoicesListPage() {
    ...
    const fetchInvoices = useCallback(() => {
        ...
        api.get('/invoices', { params })
            .then(res => {
                // Assuming backend doesn't support pagination for invoices yet
    ...
    const handleClearFilter = () => { router.push('/invoices'); };
    const handleCopyLink = (token: string) => {
        const url = `${window.location.origin}/invoice/${token}`;
        navigator.clipboard.writeText(url);
        antdMessage.success('لینک فاکتور کپی شد!');
    };
    const handlePrint = (token: string) => { window.open(`/invoice/print/${token}`, '_blank'); };
    ...
    const columns: ColumnsType<Invoice> = [
        ...
        { title: 'وضعیت', dataIndex: 'status', key: 'status', render: (status: string) => {
            const statusMap = { 'pending': { color: 'gold', text: 'در انتظار پرداخت' }, 'paid': { color: 'green', text: 'پرداخت شده' }, 'canceled': { color: 'red', text: 'لغو شده' }, 'expired': { color: 'grey', text: 'منقضی شده' } };
            const { color, text } = (statusMap as any)[status] || { color: 'default', text: status };
            return <Tag color={color}>{text}</Tag>;
        }},
        { title: 'کد رهگیری پرداخت', dataIndex: 'paymentRefId', key: 'paymentRefId', render: (refId?: string) => refId ? <Text copyable>{refId}</Text> : '-' },
        ...
        { title: 'عملیات', key: 'action', fixed: 'right', width: 180, render: (_: any, record: Invoice) => (
            <Space>
                <Tooltip title="مشاهده"><a href={`/invoice/${record.uniqueToken}`} target="_blank" rel="noopener noreferrer"><Button type="primary" icon={<EyeOutlined />} /></a></Tooltip>
    ...
    return (
        <div>
            ...
            <Search placeholder="جستجو..." onSearch={value => { setSearchTerm(value); }} onChange={e => setSearchTerm(e.target.value)} allowClear style={{ marginBottom: 16 }} />
            ...
            <Table loading={isLoading} columns={columns} dataSource={invoices} rowKey="_id" scroll={{ x: 1300 }} pagination={false} />
            <Pagination style={{ marginTop: 16, textAlign: 'center' }} current={pagination.current} pageSize={pagination.pageSize} total={pagination.total} onChange={(page, pageSize) => setPagination({ ...pagination, current: page, pageSize })} showSizeChanger />
        </div>
    );
}