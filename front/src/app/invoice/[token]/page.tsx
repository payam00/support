...
export default function PublicInvoicePage() {
    ...
    const handleApplyDiscount = async () => {
        ...
        try {
            ...
            if (res.data.invoice.finalAmount <= 0) {
                setTermsAccepted(true);
            }
    ...
    const isFree = invoice.finalAmount <= 0;
    const finalButtonText = isFree ? 'ثبت نهایی سفارش رایگان' : 'تایید و پرداخت نهایی';
    ...
    const renderPendingContent = () => {
        return (
            <>
                ...
                {invoice.allowDiscount && (
                    <>
                        {invoice.discount ? (
                            <div style={{ marginBottom: 24 }}>
                                <Text strong>تخفیف اعمال شده:</Text>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f6ffed', padding: '8px 12px', borderRadius: '6px', border: '1px solid #b7eb8f' }}>
                                    <Text code style={{ fontSize: '1rem' }}>{invoice.discount.code}</Text>
                ...
                <Card type="inner" style={{ marginBottom: 24 }}>
                    <Paragraph style={{ whiteSpace: 'pre-wrap', maxHeight: '200px', overflowY: 'auto' }}>
                        {invoice.classType.termsAndConditions}
                    </Paragraph>
                </Card>
                <Checkbox checked={termsAccepted} onChange={(e) => setTermsAccepted(e.target.checked)}>
                    قوانین و مقررات فوق را مطالعه کرده و با آن موافقم.
                </Checkbox>
                <Button 
                    type="primary" 
                    size="large" 
            ...
            case 'canceled': 
                return (
                    <Result
                        ...
                        extra={<Button type="primary" icon={<ReloadOutlined />} onClick={handlePay} loading={isProcessing}>پرداخت مجدد</Button>}
                    />
                );
            case 'pending': 
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '40px auto', padding: '20px' }}>
            <Card>
                ...
                <Title level={3}>فاکتور شناسه: {invoice._id.slice(-6)}</Title>
                <Descriptions bordered column={1}>
                    ...
                    <Descriptions.Item label="مبلغ نهایی">
                        {isFree ? 
                            <Text strong style={{ fontSize: '1.2rem', color: 'green' }}>رایگان</Text> : 
                            <Text strong style={{ fontSize: '1.2rem' }}>{invoice.finalAmount.toLocaleString()} ریال</Text>
                        }
                    </Descriptions.Item>
