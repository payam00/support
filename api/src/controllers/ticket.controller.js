...
exports.getTickets = async (req, res) => {
    try {
        ...
        const { view } = req.query; // --- MODIFIED: Read the 'view' query parameter ---
        let query = {};
        ...
        if (role === 'user') {
            ...
            // For staff (non-admin), limit query to their departments
            if (role !== 'admin' && departmentIds.length > 0) {
                 query.department = { $in: departmentIds };
            }
            // For admin, the query remains broad unless filtered by view

            // --- NEW LOGIC: Filter based on the 'view' parameter ---
            if (view === 'referred') {
                // If the frontend is asking for referred tickets, filter by that status
