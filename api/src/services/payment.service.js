...
exports.initiatePayment = async (invoiceToken) => {
    const invoice = await Invoice.findOne({ uniqueToken: invoiceToken });
     ...
     if (invoice.finalAmount <= 0) {
        console.log(`Invoice ${invoice._id} is free. Marking as paid without gateway.`);
        invoice.status = 'paid';
        invoice.paymentMethodTitle = 'رایگان';
        invoice.transaction_id = 'N/A - Free';
        
        await invoice.save();

        // If a discount was used, mark it as used
        if (invoice.discount && invoice.discount.code) {
            await Discount.updateOne({ code: invoice.discount.code }, { $inc: { timesUsed: 1 } });
        }

        // Return a special response for the frontend to handle redirection
        return {
            free: true,
            successUrl: `${process.env.FRONTEND_URL}/invoice/success/${invoice.uniqueToken}`
        };
    }
    const selectedGateway = await selectGateway();
    let amount = invoice.finalAmount;
...
exports.verifyPayment = async (queryParams) => {
    ...
    console.log("Callback received with params:", queryParams); // Added for debugging
        if (clientRefId && !paymentRefId) {
        invoice = await Invoice.findOne({ uniqueToken: clientRefId });
        if (invoice) {
            invoice.status = 'canceled';
            await invoice.save();
        }
        throw new Error('تراکنش توسط کاربر در پی‌پینگ لغو شد.');
    }
