...
const getSuggestions = async (ticket) => {
    const conversationText = formatConversation(ticket);
    const pastSolutions = await Learning.find({ department: ticket.department._id }).limit(10).sort({ createdAt: -1 });
    const formattedSolutions = pastSolutions.map(sol => `خلاصه: ${sol.ticketSummary}\nپاسخ: ${sol.successfulReply}`).join('\n---\n');
    const prompt = `
...
const getInitialAnswer = async (ticket, department) => {
    ...
    const faqsKnowledge = department.faqs.map(faq => `سوال متداول: ${faq.question}\nپاسخ آن: ${faq.answer}`).join('\n\n');
